/*
 * Copyright (C) 2020-2024 Paul Kocialkowski <contact@paulk.fr>
 *
...
void v4l2_format_setup_sizeimage(struct v4l2_format *format,
...
void v4l2_format_pixel(struct v4l2_format *format, unsigned int *width,
		       unsigned int *height, unsigned int *pixel_format)
{
	bool mplane_check;
...
void v4l2_format_pixel_format(struct v4l2_format *format,
			      unsigned int *pixel_format)
{
	bool mplane_check;
...
void v4l2_format_planes_count(struct v4l2_format *format,
			      unsigned int *planes_count)
{
	bool mplane_check;

	if (!format || !planes_count)
		return;

	mplane_check = v4l2_type_mplane_check(format->type);
	if (mplane_check)
		*planes_count = format->fmt.pix_mp.num_planes;
	else
		*planes_count = 1;
}
...
{
	return v4l2_buffers_request(video_fd, type, memory, 0);
}
...
void v4l2_buffer_setup_planes(struct v4l2_buffer *buffer,
...
int v4l2_buffer_setup_userptr(struct v4l2_buffer *buffer,
			      unsigned int plane_index, void *pointer)
{
	bool mplane_check;
	...
	if (mplane_check) {
		...
		buffer->m.planes[plane_index].m.userptr = (unsigned long)pointer;
	} else {
		if (plane_index > 0)
			return -EINVAL;

		buffer->m.userptr = (unsigned long)pointer;
	}

	return 0;
}

int v4l2_buffer_setup_fd(struct v4l2_buffer *buffer, unsigned int plane_index,
			 int fd)
{
	bool mplane_check;
	...
	if (mplane_check) {
		...
		buffer->m.planes[plane_index].m.fd = fd;
	} else {
		if (plane_index > 0)
			return -EINVAL;
